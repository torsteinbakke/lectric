...
//This is the code that controls the LED
//It is modified from the "strandtest" example code provided by Adafruit
//It uses the LPD8806 LED strip and the included library, written by Adafruit
...
void setup() {
  ...
  //Initiates the input signal from the Arduino Mega
  pinMode(5, INPUT);
  
}

// The loop plays through sequences continually as long as the signal is HIGH.
// It breaks in strategic places to make sure all LEDS are off
...
//The following code is not written by me, but are sequences provided in the example code.
